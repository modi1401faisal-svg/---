...
// =========================================================
// كود الرسوم البيانية لأداء المقاولين (مستقل تماماً)
// =========================================================
...
function drawPerformanceCharts() {
    ...
    const labels = Object.keys(contractorStats);
    const permitsData = labels.map(c => contractorStats[c].permits);
    const notesData = labels.map(c => contractorStats[c].notes);

    // 2. رسم المخطط العمودي (Bar Chart)
    const barCtx = document.getElementById('contractorBarChart');
    if (barCtx) {
    ...
    // 3. رسم المخطط الدائري (Pie Chart) للتصنيفات
...
// تشغيل الرسم البياني عند فتح التطبيق (بعد ثانيتين لضمان تحميل البيانات)
document.addEventListener("DOMContentLoaded", function() {
    setTimeout(drawPerformanceCharts, 2000);
});
