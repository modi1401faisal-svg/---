...
window.addEventListener('load', function() {
    // رابط شعار المختبر
    const labLogoUrl = "https://z-cdn-media.chatglm.cn/files/a490d9f9-e5a3-4b38-8e75-8f17d04683f0.jpeg";

    // التأكد أننا في صفحة الدخول
    const passwordInput = document.querySelector('input[type="password"]');
    if (!passwordInput) return;

    // البحث في كامل الصفحة عن العناصر التي تحتوي على صور (عدا شعار الأمانة)
    const allElements = document.querySelectorAll('body *');
    
    allElements.forEach(el => {
        // 1. استبدال الصور العادية (img)
        if (el.tagName === 'IMG' && !el.src.includes('14388a2b-5b5d-42d4-8e2f-ac93f8ede780')) {
            el.src = labLogoUrl;
            el.style.cssText = 'max-width: 150px !important; height: auto !important; margin: 0 auto 20px auto !important; display: block !important;';
        }
        
        // 2. استبدال أيقونات SVG (الرافعة قد تكون رسمة SVG)
        if (el.tagName === 'SVG') {
            const newImg = document.createElement('img');
            newImg.src = labLogoUrl;
            newImg.style.cssText = 'max-width: 150px !important; height: auto !important; margin: 0 auto 20px auto !important; display: block !important;';
            el.parentNode.replaceChild(newImg, el);
        }

        // 3. استبدال خلفيات الصور (Backgrounds)
        let bg = window.getComputedStyle(el).backgroundImage;
        if (bg && bg !== 'none' && bg.includes('url') && !el.classList.contains('main-header')) {
            el.style.setProperty('background-image', `url('${labLogoUrl}')`, 'important');
...
// مراقب لتأكيد الاستبدال إذا تأخر تحميل واجهة الدخول
